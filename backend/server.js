...
app.use(express.static(path.join(__dirname, "newApproach/chatbot-app/dist")));

// Catch-all to serve React's index.html for client-side routing
app.get("*", (req, res, next) => {
  const indexPath = path.join(__dirname, "newApproach/chatbot-app/dist/index.html");
...
// CSV parsing for charts
const parseCSVForChart = (filePath, originalName) => new Promise((resolve) => {
  const results = [];
  const headers = [];
  let isFirstRow = true;
  ...
  fs.createReadStream(filePath)
    ...
    .on("end", () => {
      ...
      resolve(generateChartConfig(results, headers, originalName));
    })
    .on("error", (err) => {
...
const generateChartConfig = (data, headers, filename) => {
  ...
  const numericColumns = headers.filter(h => data.some(r => !isNaN(parseFloat(r[h]))));
  const categoryColumns = headers.filter(h => !numericColumns.includes(h));
  if (!numericColumns.length) return null;
  
  const categoryColumn = categoryColumns[0] || headers[0];
  const categories = data.map(r => r[categoryColumn]).slice(0, 20);
  const series = numericColumns.slice(0, 3).map(col => ({
    name: col,
    data: data.slice(0, 20).map(r => parseFloat(r[col]) || 0)
  }));

  let chartType = 'column';
  if (data.length > 10 && numericColumns.length === 1) chartType = 'line';
  else if (numericColumns.length > 2) chartType = 'line';

  return { chart: { type: chartType, height: 400 }, title: { text: `Data Visualization: ${filename}`, style: { fontSize: '16px' } }, subtitle: { text: `Generated from ${data.length} data points` }, xAxis: { categories, title: { text: categoryColumn } }, yAxis: { title: { text: 'Value' } }, tooltip: { shared: true, crosshairs: true }, plotOptions: { series: { marker: { enabled: true, radius: 3 } } }, series, legend: { enabled: series.length > 1 }, credits: { enabled: false } };
};
...
app.post("/chat", upload.single("file"), async (req, res) => {
  ...
  if (!message && !file) return res.status(400).json({ success: false, message: "No message or file provided" });
  if (!OPENAI_API_KEY) return res.status(500).json({ success: false, message: "OpenAI API key is not configured." });
  ...
  try {
    ...
    if (file) {
      messageContent += `\n[Attached file: ${file.originalname}]`;
      if (file.mimetype === 'text/csv') chartConfig = await parseCSVForChart(file.path, file.originalname);
    }

    const openaiResponse = await axios.post(
      "https://api.openai.com/v1/chat/completions",
      { model: "gpt-4o-mini", messages: [{ role: "user", content: messageContent }], max_tokens: 500, temperature: 0.7 },
    ...
    if (userIdNum) {
      const chartDataString = chartConfig ? JSON.stringify(chartConfig) : null;
      if (file) {
        db.run(`INSERT INTO conversations (user_id, message, response, file_path, file_original_name, chart_data) VALUES (?, ?, ?, ?, ?, ?)`,
          [userIdNum, message || '', botReply, file.path, file.originalname, chartDataString]);
      } else {
        db.run(`INSERT INTO conversations (user_id, message, response) VALUES (?, ?, ?)`,
          [userIdNum, message, botReply]);
      }
    }

    res.json({ success: true, reply: botReply, file: file ? { originalName: file.originalname, url: `/uploads/${file.filename}` } : null, chartConfig });
  } catch (error) {
    console.error("API error:", error.response?.data || error.message);
    res.status(500).json({ success: false, message: "Failed to get response from AI service" });
  }
});
...
app.get("/conversations/:userId", (req, res) => {
  ...
  if (isNaN(userIdNum)) return res.status(400).json({ success: false, message: "Invalid userId format" });

  db.all(`SELECT * FROM conversations WHERE user_id = ? ORDER BY timestamp DESC`, [userIdNum], (err, rows) => {
    if (err) return res.status(500).json({ success: false, message: "Database error" });
    const conversations = rows.map(r => ({ ...r, chart_data: r.chart_data ? JSON.parse(r.chart_data) : null }));
